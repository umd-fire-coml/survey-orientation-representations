...
class OrientationAccuracy(tf.keras.metrics.Metric):
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        batch_sum_accuracy = tf.cast(
            self.sum_angle_accuracy(alpha_true, alpha_pred), tf.float32
        )
    ...
    def reset_state(self):
        self.num_pairs = tf.Variable(
            0, dtype=tf.dtypes.int32
        )  # num of pairs of y_true, y_pred
        # sum of accuracies for each pair of y_true, y_pred
        self.sum_accuracy = tf.Variable(0.0, dtype=TF_TYPE)
